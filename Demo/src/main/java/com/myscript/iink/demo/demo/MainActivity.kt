...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // Initialize and set click listener for gesturesHelpButton
        val gesturesHelpButton: ImageButton = findViewById(R.id.gesturesHelpButton)
        gesturesHelpButton.setOnClickListener {
            // Show a dialog with tips
            Log.d("Musa", "Help Button Pressed!")
            showHelpDialog()
        }
            ...
            is ContextualActionState.AddBlock -> {
                ...
                launchActionChoiceDialog(blockTypes.map(BlockType::toString)) { selected ->
                    when (val blockType = blockTypes[selected]) {
                        BlockType.Text -> {
                            canGesture = false
                            Log.d("Polly-showContext", canGesture.toString());
...
//        }
    ...
    // Function to show a dialog with some tips
    private fun showHelpDialog() {
        val builder = AlertDialog.Builder(this)
        builder.setTitle("Gestures Help")
        builder.setMessage("Here are some tips for using gestures in this app:\n\n1. c to undo.\n2. ↄ to redo.")
